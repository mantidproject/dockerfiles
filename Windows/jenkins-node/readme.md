...
9. Open `powershell` in administrator mode. Set docker as a service using ` &'<docker root path>\dockerd.exe' --run-service`
...
5. From the menu on the left-hand side of the page, select `Configure`. Set the preference score to `1000` and click save - this will ensure this node is used to run the test job.
